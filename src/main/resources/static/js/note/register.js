...
function checkProductDataInputValid(data) {
    const {title, content, imageCount} = data;
    ...
    if (title.length > 20) {
        throw new Error("제목은 최대 20자까지 입력 가능합니다.")
    }
...
async function saveNote() {
    ...
    const title = document.getElementById("title").value.trim(); // 앞, 뒤 공백 제거
    ...
    try {
        ...
        return alert(error.message || "오류가 발생하였습니다");
    }
}